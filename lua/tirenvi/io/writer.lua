...
local function reset_cursor_utf8(cell_pos)
    local attrs = buffer.get(0, buffer.IKEY.ATTRS)
...
function M.write(ctx, req)
    reset_cursor_utf8(req.cell_pos)
    buffer.set_lines(ctx.bufnr, req.range, req.lines, Request.is_no_undo(req), req.cell_pos)
