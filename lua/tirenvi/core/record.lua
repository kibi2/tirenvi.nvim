...
local function from_vi_line(vi_line)
    local pipe = tir_text.get_pipe_char(vi_line)
    if pipe then
...
function M.grid.new_from_vi_line(vi_line, has_continuation)
    vi_line = vi_line or ""
    local cells = tir_text.get_cells(vi_line)
...
---@return Record[]
function M.from_tir_text(vi_lines)
    local records = {}
...
---@return string[]
function M.to_tir_text(ndjsons)
    local tir_text = {}
    for _, record in ipairs(ndjsons) do
        local kind = record.kind
        if kind == CONST.KIND.PLAIN then
            tir_text[#tir_text + 1] = record.line or ""
