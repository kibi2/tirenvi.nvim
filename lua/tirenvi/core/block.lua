...
local function set_consistent_width(self)
	local width_min =
		Attr.get_width_array(Attr.grid.new(self.records[1]).columns)
	local width_max = vim.deepcopy(width_min)
	for irec = 2, #self.records do
		local widths =
			Attr.get_width_array(Attr.grid.new(self.records[irec]).columns)
...
function M.grid:inherit_neighbor_attr(attrs, row_cur)
