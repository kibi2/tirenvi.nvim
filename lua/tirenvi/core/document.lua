...
local function apply_attrs_by_range(bufdoc, attrs)
	log.assert(
		not bufdoc._tir,
		"apply_attrs_by_range should be called only for bufdoc"
	)
...
local function apply_attrs_by_id(tirdoc, c_attrs)
	log.assert(
		tirdoc._tir,
		"apply_attrs_by_id should be called only for tirdoc"
	)
...
function M.serialize_to_buf(bufdoc)
	log.assert(
		not bufdoc._tir,
		"serialize_to_buf should be called only for bufdoc"
	)
...
function M.serialize_to_flat(tirdoc)
	log.assert(
		tirdoc._tir,
		"serialize_to_flat should be called only for tirdoc"
	)
	to_flatdoc(tirdoc)
...
function M.replace_attrs(bufdoc, range, chached_attrs)
	log.assert(
		not bufdoc._tir,
		"replace_attrs should be called only for bufdoc"
	)
...
function M.infer_consistent_attr(bufdoc)
	log.assert(
		not bufdoc._tir,
		"infer_consistent_attr should be called only for bufdoc"
	)
...
function M.set_max_attr(bufdoc)
	...
	log.watch("ATTR", M.debug_attrs(bufdoc, "[5]MAX ATT:"))
end
...
function M.inherit_neighbor_attr(bufdoc, attrs, range3)
	log.assert(
		not bufdoc._tir,
		"inherit_neighbor_attr should be called only for bufdoc"
	)
...
function M.insert_empty_lines(bufdoc)
	log.assert(
		not bufdoc._tir,
		"insert_empty_lines should be called only for bufdoc"
	)
